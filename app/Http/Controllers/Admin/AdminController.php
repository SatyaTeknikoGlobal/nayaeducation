...
Class AdminController extends Controller
{
   ...
   // prd($request->toArray());
      ...
      ->editColumn('society_id', function(Admin $data) {
      ...
      ->editColumn('state', function(Admin $data) {
        $state = State::where('id',$data->state_id)->first();
        return  $state->name ?? '';
    })

      ->editColumn('city', function(Admin $data) {
...
if($request->method() == 'POST' || $request->method() == 'post'){
    ...
    if(is_numeric($id) && $id > 0){
      ...
      $rules['name'] = 'required';
        ...
        $rules['society_id'] = 'required|unique:admins,society_id';
   }
